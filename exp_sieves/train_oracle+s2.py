...
def getCategoryDicts(refexp_root, coco_root):
  ...
  for refexp_ann in refexp_anns:
    ...
    for ann_id in refexp['annotations']:
      ...
      imgid2annid[str(inst['image_id'])].add(str(inst['annotation_id']))
  ...
  return annid2imgid, annid2catid, imgid2annid

def run_filter(boxes, CNN, gold, instance_idx, imgid2annid, annid2imgid):

  final_pred = []
  filtered = []

  ann_id = str(CNN['meta'][boxes[instance_idx][gold[instance_idx][0]]])
  img_id = annid2imgid[ann_id]
  ann_list = list(set(imgid2annid[img_id]))

  cats = set()
  for ann_id in ann_list:
    cats.add(annid2catid[ann_id])

  for i in xrange(len(boxes[instance_idx])):
    ann_id = str(CNN['meta'][boxes[instance_idx][i]])
    if annid2catid[ann_id] in cats:
      final_pred.append(i)
    else:
      filtered.append(i)

  return np.array(final_pred), len(set(final_pred).intersection(set(gold[instance_idx])))

def evaluate(net, split, CNN, config, imgid2annid, annid2imgid,
  ...
  for j in pbar:
    tree = trees[j]
    filter_pred, gold_predicted = run_filter(boxes, CNN, gold, j, imgid2annid, annid2imgid)
...
annid2imgid, annid2catid, imgid2annid = getCategoryDicts(args.refexp_root, args.coco_root)
...
for ITER in range(args.epochs):
  ...
  for ii in pbar:
    tree = Xtrn_tree[ii]
    filter_pred, gold_predicted = run_filter(Xtrn_box, CNN, Ytrn, ii, imgid2annid, annid2imgid)
    if gold_predicted == 0:
      cinst += 1
      continue
    ...
    if args.val_freq != 0 and done % args.val_freq == 0:
      print("")
      val_score, val_rate = evaluate(net, dev, CNN, config, imgid2annid, annid2imgid, verbose = False)
  ...
  val_score, val_rate = evaluate(net, dev, CNN, config, imgid2annid, annid2imgid, verbose = args.verbose)
...
tst_score, tst_rate = evaluate(best_net, tst, CNN, config, imgid2annid, annid2imgid,
