...
def run_filter(box_filter_net, tree, boxes, w2i, CNN, gold, instance_idx, precision_f2):
...
def evaluate(box_filter_net, net, split, CNN, config, precision_f2, experiment_log,
             box_usage = 0, verbose = False, tst_json = [],
             out_file = '', no_lang = False):
  ...
  for j in pbar:
    tree = trees[j]
    filter_pred, gold_predicted = run_filter(box_filter_net, tree, boxes, w2i, CNN, gold, j, precision_f2)
    if gold_predicted == 0:
      n += 1
      preds.append(None)
...
for ITER in range(args.epochs):
  ...
  for ii in pbar:
    tree = Xtrn_tree[ii]
    filter_pred, gold_predicted = run_filter(box_filter_net, tree, Xtrn_box, w2i, CNN, Ytrn, ii, args.precision_f2)
    if gold_predicted == 0:
      cinst += 1
      continue
    ...
    if args.val_freq != 0 and done % args.val_freq == 0:
      print("")
      val_score, val_rate = evaluate(box_filter_net, net, dev, CNN, config, args.precision_f2, experiment_log, verbose = args.verbose)
...
tst_score, tst_rate = evaluate(box_filter_net, best_net, tst, CNN, config, args.precision_f2, experiment_log,
