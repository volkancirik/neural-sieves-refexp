...
from util.perturb import perturb
...
def evaluate(filter_net,net, split, CNN, config, precision_f1,
  ...
  for j in pbar:
    ...
    prediction  = net([w2i.get(w,0) for w in perturb([node.label for node in tree.leaves()],perturbation)], box_rep, tree)
    supporting = []
...
print("="*20)
print("Starting training {} model with perturbation: {}".format(config['model'],config['perturbation']))
...
for ITER in range(args.epochs):
  ...
  for ii in pbar:
    ...
    if args.debug_mode:
      ...
      prediction = net([w2i.get(w,0) for w in perturb([node.label for node in tree.leaves()],args.perturbation)], box_rep, tree)
      _,pred = torch.max(prediction.data,1)
