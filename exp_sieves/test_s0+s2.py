...
def evaluate(filter_net,net, split, CNN, config, precision_f1,
  ...
  for j in pbar:
    tree = trees[j]
    filter_pred, gold_predicted = run_filter(filter_net, tree, boxes, CNN, gold, j, precision_f1)
...
best_val, val_rate  = evaluate(filter_net, net, dev, CNN, config, precision_f1, verbose = args.verbose)
tst_score, tst_rate = evaluate(filter_net, net, tst, CNN, config, precision_f1, verbose = args.verbose, tst_json = tst_json, out_file = args.out_file)
