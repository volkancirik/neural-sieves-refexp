...
"""
train sieve 1 : filter boxes that have the object category same as the target object
"""
...
def evaluate(net, split, CNN, config, experiment_log, box_usage = 0, verbose = False):
  ...
  log_line = "0 : {}, 1 : {}".format(len(predict_label[0]),len(predict_label[1]))
  log.append(log_line)
  log.append("="*20)
...
for ITER in range(args.epochs):
  ...
  for ii in pbar:
    ...
    if False:#args.debug_mode:
      ...
      prediction = net([w2i.get(n.label,0) for n in tree.leaves()], box_rep)
